...
from models.dce_gtn_bert import DCE_GTN 
...
class Instructor:
    ...
    def _train(self, criterion, optimizer, max_test_acc_overall=0):
        ...
        print("\n" + "="*60)
        print("DEBUG: Checking one batch...")
        
        # 检查标签
        targets = sample_batch['polarity']
        print(f"Targets shape: {targets.shape}")
        print(f"Targets values: {targets[:10]}")
        print(f"Targets unique: {torch.unique(targets)}")
        print(f"Targets distribution: {torch.bincount(targets)}")
        ...
        with torch.no_grad():
            ...
            print(f"Outputs sample: {outputs[0, :]}")
            preds = torch.argmax(outputs, -1)
            print(f"Predictions sample: {preds[:10]}")
            print(f"Predictions unique: {torch.unique(preds)}")
    ...
    def run(self):
        ...
        if self.opt.model_name not in ['dce-gtn']:
            no_decay = ['bias', 'LayerNorm.weight'] 
            optimizer_grouped_parameters = [
                {
...
def main():
    ...
    parser.add_argument('--model_name', default='dce-gtn', type=str, help='dce-gtn')
    ...
    parser.add_argument('--graph_conv_type', type=str, default='dec-gcn', choices=['eela', 'gcn', 'gin', 'gat', 'dec-gcn'])
